...
#####
cdl_census_map <- read_excel("data/metadata/CDL_CENSUS_MAP.xlsx", sheet = "Sheet1")
...
check_coverage(c("AREA HARVESTED", "AREA BEARING & NON-BEARING"), c("AREA HARVESTED", "AREA BEARING & NON-BEARING"))
...
#8111
geoid8111 <- census[census$STATE_FIPS_CODE == 8 &
         census$COUNTY_CODE == 111] #Only total land area measure, not crop.
#26083
geoid26083 <- census[census$STATE_FIPS_CODE == 26 &
...
write.csv(census_shortdesc_map,
          "data/metadata/census_shortdesc_map.csv",
          row.names = FALSE)

# Verify all CDL census commodities are covered in the short_desc map
census_shortdesc_map <- read.csv("data/metadata/census_shortdesc_map.csv", stringsAsFactors = FALSE)
...
missing <- setdiff(cdl_commodities, unique(trimws(census_shortdesc_map$commodity)))

cat("CDL commodities with census counterpart:", length(cdl_commodities), "\n")
cat("Covered in short_desc map:", length(unique(census_shortdesc_map$commodity)), "\n")
if (length(missing) == 0) cat("All covered\n") else cat("Missing:\n", paste0("  - ", missing, "\n"))
...
county_lookup[county_lookup$GEOID_padded %in% missing_county_shortdesc & 
                !county_lookup$GEOID_padded %in% missing_geoids &
                !county_lookup$GEOID_padded %in% missing_county_areastat, ]
#manual check
#51,13
unique(census[census$STATE_FIPS_CODE==51 &
         census$COUNTY_CODE==13,]$COMMODITY) #No commodity of interest that we can map to CDL. 
...
write.csv(census_area,
          "data/CENSUS/census_area_2012.csv",
          row.names = FALSE)
#Add a csv for missing GEOIDS and the reason
CENSUS_MISSING1 <- county_lookup[county_lookup$GEOID_padded %in% missing_geoids, ] %>%
  mutate(reason = "Not in Census")

CENSUS_MISSING2 <- county_lookup[county_lookup$GEOID_padded %in% missing_county_areastat & 
                                   !county_lookup$GEOID_padded %in% missing_geoids, ] %>%
  mutate(reason = "No matching statcat")

CENSUS_MISSING3 <- county_lookup[county_lookup$GEOID_padded %in% missing_county_shortdesc & 
                                   !county_lookup$GEOID_padded %in% missing_geoids &
                                   !county_lookup$GEOID_padded %in% missing_county_areastat, ] %>%
